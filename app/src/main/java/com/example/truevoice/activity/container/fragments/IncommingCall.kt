...
import com.example.truevoice.databinding.FragmentIncommingCallBinding
import java.text.SimpleDateFormat
import java.util.Date
import java.util.Locale

class IncommingCall : Fragment() {

    private lateinit var binding: FragmentIncommingCallBinding
    ...
    private val callReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            // Check if the call state is incoming
            if (TelephonyManager.EXTRA_STATE_RINGING == state) {
                // Incoming call is ringing, fetch fresh data or update the list
                if (phoneNumber != null) {
                    fetchCallLogs()
                }
            }
    ...
    override fun onCreateView(
        ...
        binding = FragmentIncommingCallBinding.inflate(inflater, container, false)

        binding.incommingClRV.layoutManager = LinearLayoutManager(requireContext())
        callLogAdapter = CallLogAdapter(callLogs)
        binding.incommingClRV.adapter = callLogAdapter
